...
async function sendEmailOTP(email, otp, name = '') {
  await resend.emails.send({
    from: 'O Foods <onboarding@resend.dev>',
    to: email,
...
app.post('/api/register/verify-otp', async (req, res) => {
  try {
    ...
    if (!record) return res.json({ success: false, error: 'OTP expired or not found. Please request a new one.' });
    if (Date.now() > record.expires) return res.json({ success: false, error: 'OTP expired. Please request a new one.' });
    if (record.otp !== otp) return res.json({ success: false, error: 'Invalid OTP. Please try again.' });
...
app.post('/api/login/send-otp', async (req, res) => {
  try {
    ...
    if (rows.length === 0) return res.json({ success: false, error: 'No account found with this email. Please create one.' });
    ...
    res.json({ success: true, maskedEmail: maskEmail(email), hasPhone: !!user.phone });
  } catch (e) {
    console.error(e);
    res.json({ success: false, error: 'Server error. Please try again.' });
  }
...
app.post('/api/login/verify-otp', async (req, res) => {
  try {
    const { email, otp } = req.body;
    const record = otpStore.get(email);
    if (!record) return res.json({ success: false, error: 'OTP expired. Please request a new one.' });
    if (Date.now() > record.expires) return res.json({ success: false, error: 'OTP expired. Please request a new one.' });
    if (record.otp !== otp) return res.json({ success: false, error: 'Invalid OTP. Please try again.' });
...
app.delete('/api/addresses/:id', authMiddleware, async (req, res) => {
  try {
    ...
    if (check[0].is_default) {
      ...
      if (remaining.length) await db.query('UPDATE user_addresses SET is_default = 1 WHERE id = ?', [remaining[0].id]);
    }
...
app.post('/api/notify-order', authMiddleware, async (req, res) => {
  try {
    ...
    if (phone) await sendSMS(phone, `O Foods Order ${orderId} confirmed! Total: Rs.${total}. Payment: ${payment.toUpperCase()}. Delivery slot: ${slot}. Thank you!`);
...
app.post('/api/password/forgot', async (req, res) => {
  try {
    ...
    if (!rows.length) return res.json({ success: true }); // silent — prevent enumeration
      ...
      html: `
        <div style="font-family:Arial,sans-serif;max-width:500px;margin:0 auto;background:#0D0D0D;color:#F5F0E8;padding:0;border-radius:14px;overflow:hidden;">
          <div style="background:linear-gradient(135deg,#c41a1a,#6b0a0a);padding:32px 36px 28px;text-align:center;">
            <h1 style="color:#fff;font-size:30px;margin:0;">O Foods</h1>
            <p style="color:rgba(255,255,255,.65);font-size:12px;margin:6px 0 0;text-transform:uppercase;">Authentic · Handcrafted</p>
          </div>
          <div style="padding:36px;">
            ...
            <p style="font-size:14px;color:#aaa;line-height:1.7;margin:0 0 28px;">
              We received a request to reset your O Foods password. Click below to choose a new one.
            </p>
            <div style="text-align:center;margin-bottom:28px;">
              <a href="${resetUrl}" style="display:inline-block;background:#d40d0d;color:#fff;text-decoration:none;padding:16px 36px;border-radius:8px;font-weight:700;font-size:14px;text-transform:uppercase;">
                Reset My Password
              </a>
            </div>
...
app.post('/api/password/reset', async (req, res) => {
  try {
    ...
    if (!token || !newPassword) return res.json({ success: false, error: 'Token and new password are required.' });
    if (newPassword.length < 6) return res.json({ success: false, error: 'Password must be at least 6 characters.' });

    const record = resetTokenStore.get(token);
    if (!record) return res.json({ success: false, error: 'Invalid or already-used reset link. Please request a new one.' });
    ...
    if (result.affectedRows === 0) return res.json({ success: false, error: 'User not found.' });
...
app.post('/api/promo/validate', authMiddleware, async (req, res) => {
  try {
    ...
    if (usageRows.length > 0) return res.json({ valid: false, error: 'You have already used this promo code.' });

    if (promo.first_order_only) {
      const [orderRows] = await db.query('SELECT id FROM orders WHERE user_id = ? LIMIT 1', [req.user.id]);
      if (orderRows.length > 0) return res.json({ valid: false, error: 'This promo is valid only for your first order.' });
    }
    ...
    res.json({ valid: true, discount: Math.round(discount), discountType: promo.discount_type, label, code: code.toUpperCase() });
  } catch (e) {
...
app.get('/api/profile/preferences', authMiddleware, async (req, res) => {
  try {
    const [rows] = await db.query('SELECT delivery_mode, default_store_id, default_address_id FROM users WHERE id = ?', [req.user.id]);
    if (!rows.length) return res.json({ success: false, error: 'User not found.' });
    ...
    try {
      ...
      if (orderRows.length) {
        ...
        if (userRows.length && userRows[0].phone) {
          const statusMsgs = {
            dispatched: `Hi ${userRows[0].name}! Your O Foods order ${req.params.orderId} has been dispatched!`,
            delivered:  `Hi ${userRows[0].name}! Your O Foods order ${req.params.orderId} has been delivered! Enjoy! 🎉`,
            ready:      `Hi ${userRows[0].name}! Your O Foods order ${req.params.orderId} is READY for pickup!`,
          };
          if (statusMsgs[status]) await sendSMS(userRows[0].phone, statusMsgs[status]);
        }
      }
...
app.post('/api/razorpay/verify', authMiddleware, async (req, res) => {
  try {
    const { razorpay_order_id, razorpay_payment_id, razorpay_signature, items, total, delivery_address, slot, promoCode, deliveryMethod } = req.body;
    ...
    const orderId = 'OF' + Date.now().toString().slice(-6);
    await db.query(
      `INSERT INTO orders (order_id, user_id, items_json, total, store, slot, payment, delivery_address, razorpay_order_id, razorpay_payment_id, status, created_at)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,NOW())`,
      [orderId, req.user.id, JSON.stringify(items), total, delivery_address||'', slot||'4-5 business days', 'razorpay', delivery_address||'', razorpay_order_id, razorpay_payment_id, 'preparing']
    );

    // Mark promo used
    if (promoCode) {
      try {
        await db.query('INSERT INTO promo_usage (user_id, promo_code, order_id) VALUES (?,?,?)', [req.user.id, promoCode.toUpperCase(), orderId]);
        await db.query('UPDATE promo_codes SET used_count = used_count + 1 WHERE code = ?', [promoCode.toUpperCase()]);
      } catch (pe) { console.warn('Promo mark-used error:', pe.message); }
    }

    // Send confirmation email + SMS
    try {
      const [userRows] = await db.query('SELECT name, email, phone FROM users WHERE id = ?', [req.user.id]);
      if (userRows.length) {
        ...
        if (phone) await sendSMS(phone, `O Foods Order ${orderId} confirmed! Paid Rs.${total} via Razorpay. Payment ID: ${razorpay_payment_id}. Thank you!`);
      }
    } catch (notifyErr) {
...
app.listen(PORT, () => {
  ...
  console.log(`   Resend API: ${process.env.RESEND_API_KEY ? '✅ Connected' : '❌ Missing RESEND_API_KEY'}`);
  console.log(`   Razorpay:   ${process.env.RAZORPAY_KEY_ID ? '✅ Connected' : '❌ Missing RAZORPAY_KEY_ID'}\n`);
});
